...
diag_log text "";
switch (toLower(_errorType)) do
{
	case "error": {diag_log text format["******** ERROR IN %1 ********", _scriptName];};
	case "info": {diag_log text format["******** INFOMRATION IN %1 ********", _scriptName];};
	case "warning": {diag_log text format["******** WARNING IN %1 ********", _scriptName];};
};
